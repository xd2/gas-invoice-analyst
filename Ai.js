class Ai {
    ...
    static inferInvoice(blob) {
      ...
      Your response must be only the JSON object, with no additional text, no formatting, no explanation, and no backticks. The output must be directly parsable by JSON.parse().`
        ...
        // const completionString = Ai.openaiResponses(instructions, blobImage, "gpt-4o", {temperature: 0.1})
        const completionString = Ai.gemini(instructions, blob, 'gemini-2.5-pro-exp-03-25', {response_mime_type: 'application/json'})
     ...
     */
    static openaiResponses(instructions, imageBlob, model = 'gpt-4o', config = {temperature: 1}) {
        const url = 'https://api.openai.com/v1/responses';
        const apiKey = PropertiesService.getScriptProperties().getProperty('OPENAI_API_KEY');
        const base64Image = Utilities.base64Encode(imageBlob.getBytes());
        const imageMimeType = imageBlob.getContentType();
        ...
        const options = {
            method: 'post', contentType: 'application/json', headers: {
                ...
                input: [{
                    ...
                    role: "user", content: [{
                        "type": "input_image", image_url: `data:${imageMimeType};base64,${base64Image}`
                    }]
                }], ...config
        ...
        Logger.log('Tokens total, prompt, candidates %s,%s,%s', responseData.usage.total_tokens, responseData.usage.input_tokens, responseData.usage.output_tokens);

        return responseData.output[0].content[0].text
    }

    // gemini-2.0-flash gemini-2.0-flash-lite gemini-2.5-pro-exp-03-25
    static gemini(instructions, blob, model = 'gemini-2.0-flash' || 'gemini-2.0-flash-lite', config = {temperature: 1}) {
        ...
        Logger.log('Tokens total, prompt, candidates %s,%s,%s', json.usageMetadata.totalTokenCount, json.usageMetadata.promptTokenCount, json.usageMetadata.candidatesTokenCount);
